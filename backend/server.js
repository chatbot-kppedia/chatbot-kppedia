...
function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        if (err) return res.status(403).json({ error: "Sesi tidak valid atau telah berakhir." });
        req.user = user;
        next();
    });
}
    ...
    db.get(`SELECT * FROM conversations WHERE id = ? AND user_id = ?`, [conversationId, userId], (err, conv) => {
        ...
        db.run(`DELETE FROM messages WHERE conversation_id = ?`, [conversationId], (err) => {
            ...
            db.run(`DELETE FROM conversations WHERE id = ?`, [conversationId], (err) => {
                if (err) return res.status(500).json({ error: "Gagal menghapus percakapan." });
...
app.get("/api/checklist", authenticateToken, (req, res) => {
    const userId = req.user.id;
    db.all(`SELECT task_id, is_completed FROM user_checklists WHERE user_id = ?`, [userId], (err, rows) => {
        if (err) return res.status(500).json({ error: "Gagal mengambil data checklist." });
        res.json(rows);
    });
});
...
app.post("/api/checklist", authenticateToken, (req, res) => {
    ...
    db.run(query, [userId, taskId, isCompleted], (err) => {
        if (err) return res.status(500).json({ error: "Gagal menyimpan checklist." });
...
app.post("/chat", authenticateToken, async (req, res) => {
    try {
        ...
        await new Promise((resolve, reject) => {
            db.run(`INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)`, [conversationId, 'user', message], (err) => {
...
${context}`,
        ...
        // 3. Simpan pesan bot
        await new Promise((resolve, reject) => {
            db.run(`INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)`, [conversationId, 'bot', reply], (err) => {
                if (err) reject(err); else resolve();
            });
        });
