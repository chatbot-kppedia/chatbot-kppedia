...
app.post("/api/auth/login", (req, res) => {
  ...
  db.get(query, [identifier, identifier], async (err, user) => {
    ...
    if (!user || !user.password_hash) {
      return res
        .status(401)
        .json({ error: "Username/Email atau password salah." });
    }
          ...
          db.run(
            ...
            function (err) {
              if (err)
                return res
                  .status(500)
                  .json({ error: "Gagal membuat user baru dari Google." });
    ...
    const DOCUMENTS = [
      {
        ...
        url: "#",
        keywords: ["pedoman", "buku", "panduan"],
      },
      {
        id: "proposal",
        name: "Template Proposal KP",
        url: "#",
        keywords: ["proposal", "template proposal"],
      },
      {
        id: "logbook",
        name: "Form Logbook Harian",
        url: "#",
        keywords: ["logbook", "harian", "jurnal"],
      },
      {
        id: "nilai",
        name: "Form Penilaian Pembimbing",
        url: "#",
        keywords: ["nilai", "penilaian", "form nilai"],
      },
    ];
